...
export class NegativeTermsRowComponent {
  @Input() terms: Term[] = [];
  ...
  hasContextualizedTerm(term: Term): boolean {
    ...
    return MOCK_CONTEXTUALIZED_TERMS.some(ct => {
      ...
      // For negative terms, we want either undefined (not specified) or explicitly false
      if (ct.valueMetadata.isPositive !== undefined && ct.valueMetadata.isPositive !== false) {
        return false;
      }
